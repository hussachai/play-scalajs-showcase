...
object ScalaJSTodo {
  ...
  val tasks = Var(
    Seq(
      Task("TodoMVC Task A", true),
      Task("TodoMVC Task B", false),
      Task("TodoMVC Task C", false)
    )
  )
      ...
      section(id:="todoapp")(
        header(id:="header")(
          ...
          form(
            inputBox,
            onsubmit := { () =>
              tasks() = Task(inputBox.value, false) +: tasks()
              inputBox.value = ""
              false
            }
          )
                ...
                li(
                  ...
                  div(`class` := "view")(
                    ...
                    input(
                      ...
                      cursor := "pointer",
                      onchange := { () =>
                        tasks() = tasks().updated(tasks().indexOf(task), task.copy(done = !task.done))
                      },
                  ...
                  form(
                    onsubmit := { () =>
                      tasks() = tasks().updated(tasks().indexOf(task), task.copy(txt = inputRef.value))
                      editing() = None
                      false
                    },
                    inputRef
